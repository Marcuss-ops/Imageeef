				...
				if daily, ok := dataMap["daily_stats"].([]any); ok {
					for _, d := range daily {
						if m, ok := d.(map[string]any); ok {
							...
							if metricErr := analyticsStore.SaveYouTubeRevenueMetric(store.YouTubeRevenueMetric{
								...
								log.Printf("analytics: SaveYouTubeRevenueMetric (global) failed: %v", metricErr)
							}
						}
					}
				}
						...
						if daily, ok := cm["daily_stats"].(map[string]any); ok {
							for dateStr, stats := range daily {
								...
								if metricErr := analyticsStore.SaveYouTubeRevenueMetric(store.YouTubeRevenueMetric{
									...
									log.Printf("analytics: SaveYouTubeRevenueMetric (channel) failed: %v", metricErr)
								}
							}
						}
					}
