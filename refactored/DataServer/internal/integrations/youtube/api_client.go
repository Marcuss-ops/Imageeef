...
// APIClient provides YouTube Data API v3 integration
...
// GetChannelID resolves a YouTube URL or handle to a channel ID
func (c *APIClient) GetChannelID(ctx context.Context, urlOrHandle string) (string, error) {
	...
	// Parse URL to extract handle or custom name
	query := raw
	...
	if err == nil {
		...
		// Video URL - try to get channel from video
		videoID := ""
...
// SearchVideos searches YouTube videos
...
// GetRecentChannelVideos fetches recent videos from a channel
...
func (c *APIClient) fetchChannelVideos(ctx context.Context, channelID string, limit, days int) ([]Video, error) {
	...
	// Build map of video ID to title
...
// parseISO8601Duration parses PT1H2M3S format
